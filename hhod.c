...
int main(int argc, char *argv[])
{
    ...
    int hh_fd;
    hh_fd = open(MODEMDEVICE0, O_RDWR | O_NOCTTY);
    if (hh_fd < 0) {
        hh_fd = open(MODEMDEVICE1, O_RDWR | O_NOCTTY);
    }
    if (hh_fd < 0) {
        hh_fd = open(MODEMDEVICE2, O_RDWR | O_NOCTTY);
    }
    if (hh_fd < 0) {
        hh_fd = open(MODEMDEVICE3, O_RDWR | O_NOCTTY);
    }
    if (hh_fd < 0) {
       printf("Unable to find controller on USB serial port.\n"); exit(-1);
    }
